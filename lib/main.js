...
// "name" is defined by the encloseing function.  It is
...
var latencyPanel = panel.Panel({
  width: 800,
  height: 400,
  contentURL: data.url('LatencyPanel.html'),
  contentScriptFile: data.url('LatencyPanel.js'),
  onMessage: updateLatencyWindow
});

// Define the widget that, when clicked, pops-up
// the latency summar panel.
//
var latencySummary = widgets.Widget({
  id: "dpshowlatency@glezen.org",
  label: "Show Latency",
  content: 'Latency Viewer',
  width: 100,
  onClick: function() {
    ...
    worker.port.on('updateLatencyPanel', function(latencyInfo) {
      ...
      console.log('UpdateLatency event sent');
      latencyPanel.show();
    });
